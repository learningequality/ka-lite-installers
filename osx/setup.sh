...
KA_LITE="ka-lite"
KA_LITE_ZIP="$WORKING_DIR/$KA_LITE_.zip"
...
# Download PyRun.
((STEP++))
echo "$STEP/$STEPS. Downloading minimal PyRun with Python 2.7..."
if [ -d "$PYRUN_DIR" ]; then
...
# Get KA-Lite
((STEP++))
echo "$STEP/$STEPS. Downloading 'ka-lite' zip..."
